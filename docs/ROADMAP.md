...
- [ ] TypeScript + Vitest + Vite 빌드가 돌아간다
- [ ] `playground/`에 좌우 2분할 페이지가 뜬다
- [ ] `pnpm test`가 통과한다 (테스트 0개여도 됨)
- [ ] `CLAUDE.md`가 있다
- [ ] `docs/uss-reference.md`에 참조표가 있다
- [ ] `docs/backlog.md`, `docs/progress.md` 생성
- [ ] Git 초기 커밋
...
- [ ] `src/model/types.ts`에 전체 타입 정의
- [ ] 각 스타일 값의 **출처** 추적 가능 (인라인 / USS 규칙 / 상속 / 기본값)
- [ ] 원본 텍스트 정보(주석, 속성 순서, 공백) 보존 자리 확보
- [ ] 타입만 보고도 "이걸로 직렬화가 되겠다"는 확신
