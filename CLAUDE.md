...
**전용 렌더러**가 있는 컨트롤은 `VisualElement`, `Label`, `Button` **3종만**.
...
아래 넷은 S1 진행 중 매번 값을 했고, 그때그때의 판단에 맡기지 않기 위해 규칙으로 올린다.
